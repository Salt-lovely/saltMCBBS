...
(function () {
    ...
    const helpMap = {
        gid: '前往对应大区，用法：<b>gid &lt;数字ID></b>',
        fid: '前往对应版块，用法：<b>fid &lt;数字ID|英文ID></b>',
        tid: '前往对应主题，用法：<b>tid &lt;数字ID></b>',
        pid: '前往对应帖子，用法：<b>pid &lt;数字ID></b>',
        uid: '前往对应用户主页，用法：<b>uid &lt;数字ID></b>',
        uname: '前往对应用户主页，用法：<b>uname &lt;用户名></b>',
        username: '前往对应用户主页，用法：<b>username &lt;用户名></b>',
        openurl: '新窗口打开链接，用法：<b>openurl &lt;URL></b>',
        newwindow: '新窗口打开链接，用法：<b>newwindow &lt;URL></b>',
        runjs: '执行指令，用法：<b>runjs &lt;指令(可以是多行的)></b>',
        eval: '执行指令，用法：<b>eval &lt;指令(可以是多行的)></b>',
        togglenight: '切换夜间模式，用法：<b>togglenight</b>',
        togglenightstyle: '切换夜间模式，用法：<b>togglenightstyle</b>',
        nightstyle: '切换夜间模式，用法：<b>nightstyle</b>',
        cls: '清屏，用法：<b>cls</b>',
        cleanscreen: '清屏，用法：<b>cleanscreen</b>',
        editlog: '查看指定帖子的编辑记录<b>editlog &lt;pid></b>',
        posteditlog: '查看指定帖子的编辑记录<b>posteditlog &lt;pid></b>',
        test: '返回输入的参数，用法：<b>test &lt;任意内容></b>',
        echo: '返回输入的参数，用法：<b>echo &lt;任意内容></b>',
        help: '显示帮助，用法：<b>help &lt;指令名或留空></b>',
        '?': '显示帮助，用法：<b>? &lt;指令名或留空></b>',
        '？': '显示帮助，用法：<b>？ &lt;指令名或留空></b>',
    };
    // 执行指令
    ...
    function tid(cmdBody: string): string {
        ...
        if (isNaN(tid) || tid < 1) return '错误的tid';
    ...
    function pid(cmdBody: string): string {
        ...
        if (isNaN(pid) || pid < 1) return '错误的pid';
        newWindow(
            `https://www.mcbbs.net/forum.php?mod=redirect&goto=findpost&ptid=0&pid=${pid}`
        );
        return `访问pid为${pid}的帖子`;
    }
    function uid(cmdBody: string): string {
        let uid = parseInt(cmdBody);
        if (isNaN(uid) || uid < 1) return '错误的uid';
    ...
    function editlog(cmdBody: string) {
        if (cmdBody.length < 1) return '错误的pid';
        newWindow(
            `https://www.mcbbs.net/plugin.php?id=mcbbs_editlog&doing=viewthreadlogs&pid=${cmdBody}`
        );
...
(async function () {
    ...
    const db = new window.saltMCBBSDataBaseHandler(
        'saltMCBBSMemeGSU3Rk2ZWSAk0M8d'
    ); // 早早启用数据库
    const sm = window.saltMCBBS;
    const newDiv = () => {
    ...
    const insertEmoticonPanel = (function () {
        ...
        d.addEventListener('mousedown', function (this, ev) {
            ...
            pos.x = parseInt(this.style.getPropertyValue('--left') ?? '0');
            pos.y = parseInt(this.style.getPropertyValue('--top') ?? '0');
            isDrag = true;
        });
...
老版本：作者 ${m.author ?? '佚名'}，版本 ${m.version ?? '未知版本'}，表情 ${
                        m.memes.length
                    }个
新版本：作者 ${n.author ?? '佚名'}，版本 ${n.version ?? '未知版本'}，表情 ${
                        n.memes.length
...
已有版本：作者 ${n.author ?? '佚名'}，版本 ${n.version ?? '未知版本'}，表情 ${
                        n.memes.length
                    }个
导入版本：作者 ${m.author ?? '佚名'}，版本 ${m.version ?? '未知版本'}，表情 ${
                        m.memes.length
                    }个\n是否覆盖？`)
                )
                    return;
                if (
                    n.memes.length >= m.memes.length &&
                    !confirm(
                        '检测到新版本表情数量没有增加...\n此操作不可撤销，是否覆盖？'
                    )
                )
        ...
        if (typeof text == 'string' && text.length > 0) cb(text);
        else
            sm.inputBox({
                placeholder:
                    '一次只能导入一个表情包，表情包封面图是表情包最后一张图',
                acceptCallback: cb,
            });
    }
    /**导出 */
    ...
    function item(p: MemePack) {
        ...
        d.onclick = function () {
            if (chosenList.indexOf(p.name) != -1) {
                chosenList = chosenList.filter((v) => {
                    return v != p.name;
                });
        ...
        img.alt = '表情包图片';
        img.src = (
            p.memes[p.memes.length - 1] ?? {
                url: 'https://attachment.mcbbs.net/common/5f/common_110_icon.png',
            }
        ).url;
        let r = document.createElement('p');
        r.innerHTML = `<big><b>${p.name}</b></big><br>作者：${
            p.author
        }<br>许可证：${p.license}<br>
...
${p.others ? '<br>\n其他信息：' + simpleAntiXSS(p.others) : ''}`;
        ...
        input.onclick = function (e) {
            e.stopPropagation();
            if (input.checked) enableMeme(p.name);
            else disableMeme(p.name);
        };
    ...
    function insertMeme(meme: Meme) {
        ...
        if (typeof meme['width'] == 'undefined') {
            ...
            imgHTML =
                '<img src="' +
                meme.url.replace(/([\"\<\>\&])/g, '\\$1') +
                '" />';
        } else {
            //@ts-ignore
            imgStr = `[img=${meme.width},${meme.height ?? meme.width}]${
                meme.url
            }[/img]`; //@ts-ignore
            imgHTML = `<img src="${meme.url.replace(
                /([\"\<\>\&])/g,
                '\\$1'
            )}" width=${meme.width} height=${meme.height ?? meme.width} />`;
        }
        // 获取当前焦点位置
        let focus = getFocus();
        if (!focus) return;
        // 插入字符串
        if (
            focus instanceof HTMLBodyElement ||
            focus.tagName.toUpperCase() == 'BODY'
        ) {
            // 如果是所见即所得编辑框
            ...
            setTimeout(() => {
                (
                    focus as HTMLInputElement | HTMLTextAreaElement
                ).selectionStart = pos + imgStr.length;
            }, 0); // 保持选择点
        }
        /**获取应该塞入内容的元素（焦点元素） */
        function getFocus() {
            ...
            let editPage = (function () {
                ...
                if (p && p.querySelector('iframe')) return true;
                return false;
            })();
            ...
            if (a) {
                ...
                if (
                    ...
                    let iframe = document
                        ...
                        .querySelector('iframe')!; //document.querySelector('#postbox iframe')
                    if (iframe.style.display != 'none')
                        // 处于所见即所得状态
                        return a;
                    return document
                        .querySelector('#postbox')!
                        .querySelector('textarea')!; // 返回纯文本编辑框
                }
                if (a instanceof HTMLInputElement)
                    if (inputId.indexOf(a.id ?? '') != -1)
                        // 焦点处是一个单行文本框
                        // 是快速回复之类的文本框
                        return a;
            }
